...
RegularizedEvolution::RegularizedEvolution(
  ...
  : STAMP_(std::string(std::getenv("HOME"))+"/LOG/dm/md"+std::to_string(std::time(nullptr))),
    ...
    //tournament_size_(std::max(tournament_size,population_size/9+(population_size/9)/2)),
    tournament_size_(2), progress_every_(progress_every),
...
IntegerT RegularizedEvolution::Run(const IntegerT max_train_steps,
  ...
  MaybePrintProgress(true); evaluator_->ResetThreshold(2.0);
  ...
  RunHybrid(max_train_steps, max_nanos, 0.16); DimUp(2.2);
  RunHybrid(max_train_steps, max_nanos, 0.25); DimUp(2.4);  
  RunHybrid(max_train_steps, max_nanos, 0.34); DimUp(2.6); 
  RunHybrid(max_train_steps, max_nanos, 0.44); DimUp(2.8); 
  RunHybrid(max_train_steps, max_nanos, 0.50); af_.close(); of_.close(); 
  return evaluator_->GetNumTrainStepsCompleted() - start_train_steps;  
}
  
inline void RegularizedEvolution::resPOP(int new_pop) { 
  population_size_ = new_pop; tournament_size_ = 2; }
...
double RegularizedEvolution::NextFetch(const bool forced, const IntegerT fs) {
  ...
  int B[5] = {population_size_-1, population_size_-BS, population_size_-2*BS,
    population_size_-3*BS, population_size_-NP};
  for (int k = 0; k < 5; k++) B[k] = std::max(0,B[k]);
  for (int j = 0; j < cf_; j++) SecondFirst(j, cf_/2+j);
  for (int j = B[0]; j >= B[2]; j--) NextSelect(j);
#pragma omp parallel for schedule(dynamic, 1)
  for (int j = B[0]; j >= B[2]; j--) NextExecute(j, fs);
  UpdateIS(2); of_<<p; 
...
RegularizedEvolution::NextFitnessTournament(int ps) {
  ...
  const IntegerT ts = 2; //std::max(1,ps/(PS-NP)); 
